...
echo -e "${BLUE}🔄 Replacing <username> with $USER in python-blinds.service...${NC}"
...
ssh -t $USER@$DESTINATION "mkdir -p /home/$USER/workspace/python-blinds/ui/dist" > /dev/null 2>&1
scp -r ui/dist/* $USER@$DESTINATION:/home/$USER/workspace/python-blinds/ui/dist
...
echo -e "${BLUE}🧹 Removing legacy Node webserver service...${NC}"
ssh -t $USER@$DESTINATION "sudo rm -f /lib/systemd/system/python-blinds-webserver.service" > /dev/null 2>&1 && echo -e "${GREEN}✅ legacy webserver service removed${NC}"
